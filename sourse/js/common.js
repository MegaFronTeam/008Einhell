...
function eventHandler() {
  ...
  const headerBlockNavWrap = document.querySelector(
    ".headerBlock__caption-container"
  );
  ...
  if (compareWrap) {
    let compareSwiper = new Swiper(
      compareWrap.querySelector(".compare-slider"),
      {
        slidesPerView: 2,
        spaceBetween: 10,
        breakpoints: {
  ...
  if (filter) {
    ...
    document.addEventListener("click", function (event) {
      ...
      if (
        ...
        !target.closest(".sCatalog__filter-btn--js")
      ) {
        filter.classList.remove("active");
        body.classList.remove("fixed2");
      }
  ...
  function comparePate() {
    ...
    let topHead = document
      .querySelector(".sCompare__main-head")
